...
import { Bot, Sparkles, Lightbulb, RefreshCw, CheckCircle2 } from "lucide-react";
...
export function AIStudyCoach({
  ...
  const realTips = [
    ...
    avgNet >= 60
      ? "🌟 Net ortalamanız LGS üst hedefleri için harika bir seviyede! Matematik ve Fen soru çözümlerine ağırlık vererek Galatasaray/İstanbul Erkek barajını yakalayabilirsiniz."
      : "💡 İpucu: Fen Bilimleri ve Türkçe paragraf sorularında soru kökünde altı çizili kelimelere dikkat etmek netlerinizi ortalama +3.5 artıracaktır.",
    "🎯 LGS sınavına hazırlıkta günlük 25 dakikalık Pomodoro odaklanma seanslarıyla çalışma veriminizi %40 artırabilirsiniz.",
  ];

  const [currentTipIndex, setCurrentTipIndex] = useState(0);

  const nextTip = () => {
    setCurrentTipIndex((prev) => (prev + 1) % realTips.length);
  };
    ...
    <div className={`p-4 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white shadow-xl border border-indigo-800/60 relative overflow-hidden ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-indigo-800/40 pb-2.5 mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-indigo-200">Optik netlerinize göre canlı yapay zeka koçluk tavsiyeleri</p>
          </div>
        </div>
      ...
      <div className="p-3 rounded-xl bg-white/5 border border-white/10 text-xs space-y-2 mb-3">
        <div className="flex items-center gap-1 text-amber-300 font-bold text-[11px]">
          <Lightbulb className="w-3.5 h-3.5" /> Koçunuzun Canlı Tavsiyesi:
        </div>
        <p className="text-slate-200 leading-relaxed font-medium">
          {realTips[currentTipIndex]}
        </p>
      </div>
